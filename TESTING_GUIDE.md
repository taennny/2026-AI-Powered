...
3. 지도를 꾹 누르면 확대 모달이 뜬다 (공유 버튼은 현재 비활성)
...
| 카카오 로그인    | `.env`에 `EXPO_PUBLIC_KAKAO_REST_API_KEY`가 없으면 버튼이 막힘  |
| 지도 공유        | 현재 비활성 (`RNFetchBlob`이 Expo 웹 번들을 깨뜨려 주석 처리)   |
| AI 타임라인 생성 | AI 서버 URL 미설정 시 백엔드가 임시 응답을 반환               |
| 프론트 테스트    | `cd frontend && npm test` (jest-expo, 순수 로직만)              |
